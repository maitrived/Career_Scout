...
from python.db.client import get_job, get_application_by_job, get_connection, mark_applied
...
async def fill_by_selector_or_label(page: Page, selectors: List[str], label_texts: List[str], value: str) -> bool:
    """
    ...
    for sel in selectors:
        try:
            ...
            if await loc.count() > 0 and await loc.first.is_visible() and await loc.first.is_editable():
...
async def upload_file_by_selector_or_label(page: Page, selectors: List[str], label_texts: List[str], file_path: str) -> bool:
    """
...
async def autofill_greenhouse(page: Page, resume: dict, pdf_path: str, cover_letter: str):
    ...
    # Basic Info
    await fill_by_selector_or_label(page, ["input#first_name"], ["First Name"], first_name)
    await fill_by_selector_or_label(page, ["input#last_name"], ["Last Name"], last_name)
    await fill_by_selector_or_label(page, ["input#email"], ["Email"], contact.get("email", ""))
    await fill_by_selector_or_label(page, ["input#phone"], ["Phone"], contact.get("phone", ""))
    ...
    uploaded = await upload_file_by_selector_or_label(
        page, 
        ["input#resume", "input[type='file'][id*='resume']", "input[type='file'][accept*='pdf']"], 
        ["Resume", "CV"], 
        pdf_path
    )
    if uploaded:
    ...
    cl_filled = await fill_by_selector_or_label(
        page, 
        ["textarea#cover_letter_text", "textarea#cover_letter", "textarea[name*='cover_letter']"], 
        ["Cover Letter"], 
        cover_letter
    )
    if cl_filled:
    ...
    # Links
    await fill_by_selector_or_label(page, ["input[autocomplete*='linkedin']", "input[name*='linkedin']"], ["LinkedIn"], contact.get("linkedin", ""))
    await fill_by_selector_or_label(page, ["input[autocomplete*='github']", "input[name*='github']"], ["GitHub"], contact.get("github", ""))
    await fill_by_selector_or_label(page, ["input[name*='website']", "input[name*='portfolio']"], ["Portfolio", "Website"], contact.get("portfolio", ""))

async def autofill_lever(page: Page, resume: dict, pdf_path: str, cover_letter: str):
    """Fills out standard Lever board elements."""
    contact = resume.get("contact", {})

    # Lever uses 'name' full instead of first/last
    await fill_by_selector_or_label(page, ["input[name='name']"], ["Full Name", "Name"], resume.get("name", ""))
    await fill_by_selector_or_label(page, ["input[name='email']"], ["Email"], contact.get("email", ""))
    await fill_by_selector_or_label(page, ["input[name='phone']"], ["Phone"], contact.get("phone", ""))
    await fill_by_selector_or_label(page, ["input[name='org']"], ["Current company", "Organization"], "Vybd (formerly BulkMagic)")
    ...
    # Links
    await fill_by_selector_or_label(page, ["input[name='urls[LinkedIn]']"], ["LinkedIn"], contact.get("linkedin", ""))
    await fill_by_selector_or_label(page, ["input[name='urls[GitHub]']"], ["GitHub"], contact.get("github", ""))
    await fill_by_selector_or_label(page, ["input[name='urls[Portfolio]']", "input[name='urls[Twitter]']"], ["Portfolio", "Website"], contact.get("portfolio", ""))
...
async def autofill_ashby(page: Page, resume: dict, pdf_path: str, cover_letter: str):
    ...
    await fill_by_selector_or_label(page, ["input[name*='firstName']", "input#firstName"], ["First Name"], first_name)
    await fill_by_selector_or_label(page, ["input[name*='lastName']", "input#lastName"], ["Last Name"], last_name)
    await fill_by_selector_or_label(page, ["input[name*='email']", "input#email", "input[type='email']"], ["Email"], contact.get("email", ""))
    await fill_by_selector_or_label(page, ["input[name*='phone']", "input#phone", "input[type='tel']"], ["Phone"], contact.get("phone", ""))
    ...
    # Links
    await fill_by_selector_or_label(page, ["input[name*='linkedin']"], ["LinkedIn"], contact.get("linkedin", ""))
    await fill_by_selector_or_label(page, ["input[name*='github']"], ["GitHub"], contact.get("github", ""))
    await fill_by_selector_or_label(page, ["input[name*='portfolio']", "input[name*='website']"], ["Portfolio", "Website"], contact.get("portfolio", ""))
...
async def autofill_job_by_url(page: Page, job: Job, resume_json: dict, pdf_path: str, cover_letter: str):
    """
    ...
    if "greenhouse.io" in url or "boards.greenhouse.io" in url:
        await autofill_greenhouse(page, resume_json, pdf_path, cover_letter)
    elif "lever.co" in url or "jobs.lever.co" in url:
        await autofill_lever(page, resume_json, pdf_path, cover_letter)
    elif "ashbyhq.com" in url or "jobs.ashbyhq.com" in url:
        await autofill_ashby(page, resume_json, pdf_path, cover_letter)
    else:
        # Generic fallback
        logger.warning("Unrecognized domain. Applying fallback form filling...")
        contact = resume_json.get("contact", {})
        await fill_by_selector_or_label(page, [], ["First Name", "Name"], resume_json.get("name", ""))
        await fill_by_selector_or_label(page, [], ["Last Name"], resume_json.get("name", "").split(" ")[-1])
...
async def run_multi_tab_autofill(job_ids: List[str]):
    ...
    for job_id in job_ids:
        ...
        if not app or not app.resume_version_id:
            logger.error(f"No tailored application version found for job '{job.title}' ({job_id}). Please tailor first.")
            continue
        ...
        if not row or not row['pdf_path']:
            logger.error(f"Tailored PDF path not found for application version {app.resume_version_id}.")
            continue
    ...
    logger.info(f"Starting Playwright headful session to fill {len(jobs_data)} applications...")
    ...
    async with async_playwright() as p:
        ...
        for index, item in enumerate(jobs_data):
            ...
            logger.info(f"[{index + 1}/{len(jobs_data)}] Opening tab for: {job.title} at {job.company}")
            ...
            try:
                await autofill_job_by_url(page, job, resume_json, pdf_path, cover_letter)
            except Exception as ex:
        ...
        print("Please click through each tab in the opened Chrome browser window, answer custom questions, and click submit.")
        print("="*80 + "\n")
        
        # Block until user hits Enter in the console
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, input, "Press [Enter] in this terminal when you have finished submitting ALL applications to close the browser...")
        ...
        for item in jobs_data:
            ...
            while user_confirm.lower() not in ["y", "n"]:
                user_confirm = input(f"Did you successfully submit application for '{job.title}' at '{job.company}'? (y/n): ")
